...
mod tests {
    ...
    use crate::auth::{
        add_user, create_user_session, find_user, login, sign_up, User,
    ...
    async fn test_find_user() {
        ...
        match result {
            ...
            Err(err) => {
                panic!(
                    "Failed finding user with username {} with  root cause {}",
                    mock_user.username, err
                );
            }
        }
    }
    ...
    async fn test_login() {
        ...
        let result = login(&String::from("test5"), &String::from("unhashed"), &pool).await;
    ...
    async fn test_create_user_session() {
        ...
        match result {
            Ok(created_session) => {
                ...
                let current_time = Local::now() + Duration::minutes(DEFAULT_SESSION_DURATION_MIN)
                    - Duration::minutes(1);
                assert!(
                    created_session
                        .expiry_date
                        .unwrap()
                        .cmp(&current_time.naive_utc())
                        == Ordering::Greater
                )
            }
